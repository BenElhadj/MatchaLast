  ...
  mutations: {
    SOCKET_connect (state) {
      if (!state.isConnected && state.user.id) {
        const app = createApp({})
        app.config.globalProperties.$socket = io(`${process.env.VUE_APP_API_URL}`)
      ...
      try {
        ...
        if (data.id_conversation === state.selectedConvo) {
          if (!state.newMessage) {
            ...
            const url = `${process.env.VUE_APP_API_URL}/api/chat/update`
          ...
          const url = `${process.env.VUE_APP_API_URL}/api/notif/add`
